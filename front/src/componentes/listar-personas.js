...
const ListarPersonas = () => {
    ...
    const eliminar = (id) => {
        BackService.eliminarPersona(id).then(() => {
            listarPersonas();
        });
    };
    ...
    return (
        <div style={{ margin: '25px 15px' }}>
            ...
            <div style={{textAlign: 'end'}}>
                <button className="btn btn-success" onClick={crear}>Crear Persona</button>
            </div>
            {personas.length === 0 ?(
                <p>Cargando...</p>
            ) : (
            <div>
                <table className="table">
                    ...
                    {personas.map((dato) => (
                        <tr key={dato.id}>
                            <td className="text-center">{dato.nombrecompleto}</td>
                            <td className="text-center">{dato.nrodocumento}</td>
                            <td className="text-center">{dato.telefono}</td>
                            <td className="text-center">{dato.correo}</td>
